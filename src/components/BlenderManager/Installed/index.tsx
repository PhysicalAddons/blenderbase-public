import { Button, DataTable, OverflowMenu, OverflowMenuItem, Table, TableBody, TableCell, TableContainer, TableHead, TableHeader, TableRow, TableToolbar, TableToolbarContent, TableToolbarSearch } from '@carbon/react';
import { Reset, Rocket } from '@carbon/react/icons';
...
const InstallBlenderTable = () => {
	// const [blenderVersionBuildTypesFilters, setblenderVersionBuildTypesFilters] = useState<IBlenderVersionInstallBuildTypeFilter[]>([])
	const { installedBuilds, setInstalledBuilds } = useBlenderManagerStore()
	// const { setActiveInstallBuildType } = useBlenderManagerStore()
	useEffect(() => {
		async function init() {
	...
	// const changeFilter = async (selectedItem: IBlenderVersionInstallBuildTypeFilter) => {
	// 	try {
	// 		await blenderService.updateInstallBlenderBuildType(selectedItem);
	// 		setActiveInstallBuildType(selectedItem)
	// 	} catch (e) {
	// 		console.error(e);
	// 	}
	// }
	const deleteInstalledBlender = async (id: string) => {
		try {
		...
		<div>
			<div className='blender_version_table'>
				<DataTable
					isSortable={true}
					rows={
						installedBuilds.map((data: IBlenderVersion, index: number) => ({
							id: index.toString(),
							version: data.version,
							variant: data.risk_id,
							information: data,
							actions: data
						}))
					}
					headers={[
						...
						{ key: 'information', header: 'Information' },
					...
					{({
						...
						<TableContainer {...getTableContainerProps()}>
							<TableToolbar {...getToolbarProps()} aria-label="data table toolbar">
								<TableToolbarContent>
									...
									<Button
										kind="ghost"
										renderIcon={Reset}
										iconDescription="Refresh database"
										title="Refresh database"
										hasIconOnly
										onClick={async () => await setInstalledBuilds()}
									/>
									{/* <Dropdown
										size="lg"
										id="default"
										...
										initialSelectedItem={activeInstallBuildType} // ({ text: selectedBlenderVersionBuildTypeFilter?.text })
							...
							<Table {...getTableProps()}>
								...
								{rows.length === 0 ?
									<TableBody>
										<TableRow className='empty_installed_blender_table'>
											<TableCell key={1}>
												<>No installed Blenders found.</>
											</TableCell>
											<TableCell key={2}>
											</TableCell>
											<TableCell key={3}>
									...
									<TableBody>
										{rows.map((row) => (
											<TableRow {...getRowProps({ row })} key={row.id}
												className={
													`blender_manager_installed_table_row 
													${((row.cells.find(c => c.info.header === "information")?.value as IBlenderVersion)?.download_status_type_id ?? 0) !== 4
														? "disabled"
														: "enabled"
													}`
												}
											>
												{row.cells.map((cell) => (
													<TableCell key={cell.id} aria-disabled>
														{cell.info.header === 'version' ?
															(
															...
															) : cell.info.header === 'variant' ? (
																<>
																	<div
																		className={`blender_tag ${cell.value}`}
																		title={`${cell.value} variant`}
																	>
																		{cell.value}
																	</div>
																</>
															) : cell.info.header === 'information' ? (
																<div
																	className='blender_meta'
																>
																	<div>
																		{(cell.value as IBlenderVersion).architecture}
																	</div>
																	<div>
																		<span>
																			{(cell.value as IBlenderVersion).file_mtime === 0 ? "" : (cell.value as IBlenderVersion).file_mtime}
																		</span>
																	</div>
																	<div>
																		{(cell.value as IBlenderVersion).hash_url ? (
																			<a target="_blank" href={(cell.value as IBlenderVersion).hash_url!}>
																				{(cell.value as IBlenderVersion).hash}
																			</a>
																		) : (
																			(cell.value as IBlenderVersion).hash && (
																				<span>{(cell.value as IBlenderVersion).hash}</span>
																			)
																		)}
																	</div>
																	<div>
																		{(cell.value as IBlenderVersion).patch_url && (
																			<a target="_blank" href={(cell.value as IBlenderVersion).patch_url!}>
																				{(cell.value as IBlenderVersion).patch}
																			</a>
																		)}
																	</div>
																	<div>
																		<div>
																			{(cell.value as IBlenderVersion).url && (
																				<a target="_blank" href={(cell.value as IBlenderVersion).url!}>
																					{(cell.value as IBlenderVersion).file_extension}
																				</a>
																			)}
																		</div>
																	</div>
																...
																<div>
																	<Button
																		kind="secondary"
																		title={`Launch ${(cell.value as IBlenderVersion).version} ${(cell.value as IBlenderVersion).risk_id}`}
																		renderIcon={Rocket}
																		className='launch_button'
																		onClick={async () => blenderService.launchInstalledBlender((cell.value as IBlenderVersion).id)}
																	>
																		Launch
																	</Button>
																	<OverflowMenu flipped>
																		<OverflowMenuItem
																			requireTitle={true}
																			title={`Delete Blender ${cell.value.version} ${cell.value.variant_type}`}
																			isDelete
																			itemText="Delete Blender"
																			onClick={async () => await deleteInstalledBlender((cell.value as IBlenderVersion).id)}
																		/>
																	</OverflowMenu>
																</div>
															) : (
...
export default InstallBlenderTable